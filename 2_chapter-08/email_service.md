...
<img src="/2_chapter-08/img/email_received_arch.png" alt="arch-3" width="70%">
...
- 상용화된 DB 중에는 적합한 DB가 없고, 대형 메일 서비스업체는 내부 구조를 공개하지 않으므로 주요 조건을 알아두자.
...
| 비교 항목    | ElasticSearch            | 맞춤형 검색 엔진 (직접 개발)           |
| ---------- | ------------------------- | ----------------------------------- |
...
| 3. 데이터 일관성    | 한 데이터에 두 사본이 존재(저장소와 ElasticSearch)하여 데이터 일관성 유지 까다로움 | 기존 저장소에 사본 유지하는 일관성만 체크 |
| 4. 데이터 손실 가능성 | 색인이 손상되면 주 저장소의 데이터를 사용해 복구 가능 |   |
